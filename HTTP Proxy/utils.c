...
int sendrequest(int sd) 
{
  ...
  if (servhost && servport) {
      ...
      ushort servportvar = atoi(servport);
      int servsd = connecttoserver(servhost, servportvar);
      write(servsd, msg, len);
...
char *readresponse(int sd)
{
    ...
    char buffer[8192];
    while((n = read(sd, buffer, 8192)) > 0){
    	memcpy(&msg[len], buffer, n);
    	len += n;
    	bzero(buffer, 8192);
    }
